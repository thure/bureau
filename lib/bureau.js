        ...
        if(cssAnimations){
          ...
          if(!usingZepto /* using jQuery */){
            if(showHide === 'show'){
              var q;
              for (q in props){
                if (props.hasOwnProperty(q)){
                  $(this).css(q, '0');
                }
              }
              $(this).animate(props, durationMS, easing, function(){
                var p, clear = {};
                for (p in props){
                  if (props.hasOwnProperty(p)){
                    clear[p] = '';
          ...
          //else? otherwise we'd have to design an animation function ourselves, which is out of scope.
        }
      };
    };
