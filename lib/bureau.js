    ...
    var generateBinding = function (rules, $responding, $watched) {
      ...
      return function (state) {
        ...
        if ($watched.data('previous-validation') !== validation.toString()){
          ...
          if(validation){
            if(state.data === 'init' && exists(callbacks['init-yup'])) {callbacks['init-yup'].call($responding, $watched);}else{
              if(exists(callbacks.yup)) {callbacks.yup.call($responding, $watched);}
            }
          }else{
            if(state.data === 'init' && exists(callbacks['init-nope'])) {callbacks['init-nope'].call($responding, $watched);}else{
              if(exists(callbacks.nope)) {callbacks.nope.call($responding, $watched);}
            }
          }
        }
        if (validation) {
          if(state.data === 'init' && exists(callbacks['init-everyYup'])) {callbacks['init-everyYup'].call($responding, $watched);}else{
            if(exists(callbacks.everyYup))  {callbacks.everyYup.call($responding, $watched);}
          }
        } else {
          if(state.data === 'init' && exists(callbacks['init-everyNope'])) {callbacks['init-everyNope'].call($responding, $watched);}else{
