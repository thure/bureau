...
(function ($) {
  ...
  var cssAnimations = usingModernizr && exists(window.Modernizr.cssanimations) ? window.Modernizr.cssanimations : function () {
  ...
  $.fn.bureau = function (configuration) {
    ...
    var defaultTrigger = 'change',
      ...
      possibleCallbacks = ['then', 'else'];

    var animate = function (properties, duration, easing, callback) {
      if(cssAnimations){
        /*TODO: animate this using CSS transitions */
      }else{
    ...
    var generateBinding = function (rules, $responding, $watched) {
      ...
      return function (e) {
        ...
        if (validation) {
          if(exists(callbacks.then)) {callbacks.then.call($responding, $watched);}
        } else {
          if(exists(callbacks['else'])) {callbacks['else'].call($responding, $watched);}
        }
      };
